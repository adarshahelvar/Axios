...
function getTodos() {
  ...
  function updateTodo() {
    axios
//use patch to update the data
//use .put to completly change the data

    .patch('https://jsonplaceholder.typicode.com/todos/1',{
        title: 'patch updated the completly',
        completed:true
    })
    // .put('https://jsonplaceholder.typicode.com/todos/1',{
    //     title: 'put changed completly',
    //     completed:true
    // })
    .then(res => showOutput(res))
    .catch(err => console.error(err));
  ...
  function getData() {
    ...
    .then(axios.spread((x,y)=>showOutput(y)));
  }
  ...
  function cancelToken() {
    ...
    .catch(thrown=>{
        if(axios.isCancel(thrown)) {
            console.log('cancelled request',thrown,message);
        }
    });
    if(true){
